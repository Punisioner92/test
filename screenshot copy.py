...
def area_sel():
    ...
    bgimage = ImageEnhance.Brightness(image).enhance(1.0)  # değer sıfıra yakalaşınca ekranın seçilmeyen tarafını karartıp seçilen kısmın görüntüsünü alır
    ...
    #if roi_image: çekilen resmin görüntülenmesi 
        #show_image(roi_image) karoları kaldır
    roi_image.save('screenshot.png', 'PNG')

tk.Button(root, text='select area', width=30, height=10, command=area_sel).pack()
